...
Denne filen beskriver **selve templaten** (`claude-next.js-template`). Den slettes automatisk i oppstart-steg 10, slik at det klonede prosjektet ikke ender opp med template-dokumentasjon.
...
`mitt-nye-prosjekt` blir en ny undermappe av `C:\Kodeprosjekter\` med templaten kopiert inn — **uten** templatens git-historikk. Du starter med en ren mappe som steg 09 initialiserer git på.
...
Steg 09 kjører `rm -rf .git && git init` i dette tilfellet, som gir samme sluttresultat (fersk historikk) — bare litt mer trafikk ved kloning.
...
6. Gjennomgår Supabase: prosjekt-opprettelse, klienter, Auth (login/signup/RLS), CLI for migrations, chainer session-proxy etter i18n-proxy
7. Lager `.env.example` med dokumenterte nøkler
8. Verifiserer at `dev`, `build`, `lint`, `typecheck` er grønne
9. Resetter git-historikk, gjennomgår `gh auth` + repo-opprettelse, (valgfritt) Vercel-linking
10. Rydder opp (sletter `oppstart/` + TEMPLATE.md, final commit, pusher)

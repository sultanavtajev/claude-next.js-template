...
Next.js og tilhørende stack (React, Supabase, shadcn) endrer seg ofte. Denne skillen tvinger et "research first"-steg før implementasjon.
...
- Du er usikker på om en pattern er fortsatt gjeldende (f.eks. `getSession()` vs `getUser()` i Supabase).
...
- `anon key` → `publishable key` (Supabase)
- `getSession()` → `getUser()` (Supabase server-side auth)
...
- Bruke `supabase.auth.getSession()` for server-side auth-sjekk (bruk `getUser()`).

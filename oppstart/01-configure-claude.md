...
- `{{GITHUB_REPO}}` → steg 10 (git + GitHub-repo-opprettelse)
- `{{VERCEL_PROJECT}}` → steg 10 (Vercel-linking)
...
- `TEMPLATE.md` — dokumentasjon for selve templaten, slettes i steg 11.
...
Vi ekskluderer `TEMPLATE.md` fra sjekken fordi filen inneholder dokumentasjon om templaten (inklusive eksempel-placeholders), og slettes uansett i steg 11.
...
- **Grep finner treff i `oppstart/`**: det er OK — denne mappen slettes i steg 11.
- **Grep finner `{{GITHUB_REPO}}`, `{{VERCEL_PROJECT}}`, `{{SUPABASE_PROJECT_REF}}`**: forventet — de fylles inn i steg 07 og 10.
